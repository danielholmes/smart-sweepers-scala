...
class CNeuralNet() {
  ...
  private val layers = new util.Vector[SNeuronLayer]
  ...
  private def CreateNet() {
    ...
    if (numHiddenLayers > 0) {
      ...
      layers.add(new SNeuronLayer(m_NeuronsPerHiddenLyr, m_NumInputs))
      var i: Int = 0
      while (i < numHiddenLayers - 1) {
        {
          layers.add(new SNeuronLayer(m_NeuronsPerHiddenLyr, m_NeuronsPerHiddenLyr))
        }
        {
          i += 1; i
        }
      }
      layers.add(new SNeuronLayer(m_NumOutputs, m_NeuronsPerHiddenLyr))
    }
    else layers.add(new SNeuronLayer(m_NumOutputs, m_NumInputs))
  }
  ...
  def putWeights(weights: util.Vector[Double]) {
    ...
    for (i <- 0 to numHiddenLayers) {
      for (j <- 0 until layers.get(i).numNeurons) {
        val newWeights = new util.Vector[Double]
        for (k <- 0 until layers.get(i).neurons.get(j).numInputs) {
          newWeights.add(weights.get(cWeight))
          cWeight += 1
        }
        layers.get(i).neurons.set(j, new Neuron(newWeights.asScala.toList))
      }
    }
  }
  ...
  def Update(firstInputs: util.Vector[Double]): util.Vector[Double] = {
    ...
    for (i <- 0 to numHiddenLayers) {
      ...
      for (j <- 0 until layers.get(i).numNeurons) {
        val neuron = layer.neurons.get(j)
        outputs.add(neuron.getActivation(inputs.subList(cWeight, cWeight + neuron.numInputs - 1).asScala.toList))
        cWeight = 0
      }
    }
    outputs
